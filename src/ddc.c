...
uint16_t read_vcp(i2c_master_dev_handle_t dev_handle, uint8_t operation, uint16_t fallback_value)
{
  ...
  data[6] = (I2C_ADDRESS << 1) ^ data[0] ^ data[1] ^ data[2] ^ data[3];
...
void ddc_task(void *param)
{
  ...
  while (true)
  {
    uint16_t brightness = read_vcp(dev_handle, 0x10, last_brightness);
    uint16_t input = read_vcp(dev_handle, 0x60, last_input);
