...
uint16_t read_vcp(i2c_master_dev_handle_t dev_handle, uint8_t operation)
{
  uint8_t data[] = {
      0x51,
      0x82,
      0x01,
      operation,
      (I2C_ADDRESS << 1) ^ 0x51 ^ 0x82 ^ 0x01 ^ operation,
  };
...
void write_vcp(i2c_master_dev_handle_t dev_handle, uint8_t operation, uint16_t value)
{
  uint8_t data[] = {
      ...
      value >> 8,
      value,
      (I2C_ADDRESS << 1) ^ 0x51 ^ 0x84 ^ 0x03 ^ operation ^ (value >> 8) ^ value,
  };
  ESP_ERROR_CHECK(i2c_master_transmit(dev_handle, data, sizeof(data), 500));
}
...
void ddc_task(void *param)
{
  ...
  while (true)
  {
    uint16_t brightness = read_vcp(dev_handle, 0x10);
    uint16_t input = read_vcp(dev_handle, 0x60);
